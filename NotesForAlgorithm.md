...
```C++
//计算行数m和列数n
	int n = (int)sqrt(N); //n是小的那个，用面积的开方来控制
	while(N%n != 0){
		n--; //面积能被n整除，较小的那个的最大值
	} 
	int m = N/n; //自动满足 m>=n 
...
bool visited[MAXN] = {0};
//有的时候需要设置一个bool数组在判断该地址上是否已经存储了节点
//当然也可以如此初始化 visited 数组
...
//%05d以十进制整数形式输出第二个参数list[0].addr并指定最小宽度为5不足5位时用'0'在左边填充
```

```c++
cout << list.size() << " " << setw(5) << setfill('0') << list[0].addr << endl;
//setw(5)流操纵（<iomanip> 头文件），设置下一个输出项的最小宽度为5个字符。如果输出的内容不足5位，则用填充字符补齐，填充字符默认为格，但这里被后续的setfill改变了
//setfill('0')：流操纵符，设置填充字符为'0'。它会影响之后所有输出项的填充行为，直到再次被修改
```
...
| 函数               | 说明                                   | 时间复杂度 |
| :----------------- | :------------------------------------- | :--------- |
| `push(const T& x)` | 将元素 `x` 压入栈顶                    | O(1)       |
| `pop()`            | 移除栈顶元素（**不返回**被移除的元素） | O(1)       |
| `top()`            | 返回栈顶元素的**引用**（可读可写）     | O(1)       |
| `empty()`          | 判断栈是否为空，空则返回 `true`        | O(1)       |
| `size()`           | 返回栈中元素的个数                     | O(1)       |

**注意**：`pop()` 不返回元素，若要获取并移除栈顶元素，需先调用 `top()` 再调用 `pop()`
